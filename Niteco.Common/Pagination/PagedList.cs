...
namespace Niteco.Common.Pagination
{
    public class PagedList<T> : List<T>, IPagedList<T>
    {
        public PagedList(IEnumerable<T> source, int index, int pageSize, int? virtualCount = null)
            : this(source.AsQueryable(), index, pageSize, virtualCount)
        {
            //if (index < 0)
            ...
            //PageSize = pageSize;
            //PageIndex = index;
            //TotalItemCount = virtualCount.HasValue ? virtualCount.Value : realTotalCount;
        ...
        public PagedList(IQueryable<T> source, int index, int pageSize, int? virtualCount = null)
        {
            if (index < 0)
                throw new ArgumentOutOfRangeException("index", "Value can not be below 0.");
            if (pageSize < 1)
