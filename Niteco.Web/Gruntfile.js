module.exports = function (grunt) {
    ...
    var date = new Date();
    var time = date.getUTCDate().toString() + date.getUTCMonth().toString() + date.getUTCFullYear().toString();
    ...
    grunt.initConfig({
        ...
        requirejs: {
            ...
            deploy: {
                options: {
                    out: '<%= project.dirs.scripts.out %>/scripts' + time + '.js',
    ...
    // CSS Tasks
    grunt.registerTask('css', ["sass:dev", "autoprefixer", "cssmin"]);
    ...
    grunt.registerTask('deployjs', ["requirejs:deploy_nonver","requirejs:deploy","uglify"]);

    //grunt.registerTask('build', ['clean', 'assemble', "sass:dev", "cssmin", "requirejs:dev", "copy","watch"]);
    grunt.registerTask('build', ['clean', 'static', "css", "js", "copy","deployjs"]);
    //grunt.registerTask('build', ["htmllint"]);
    grunt.registerTask('buildquick', ['clean', 'static', "css", "js", "copy"]);
